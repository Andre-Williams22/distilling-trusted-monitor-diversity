#!/usr/bin/env bash
# M6: MACA's MV-SFT on the debate's winning answers (ADR-0009). Run ON THE VM
# after infra/vast/remote_setup.sh, inside tmux:
#
#   cd ~ && tmux new -d -s m6 'bash infra/vast/run_m6.sh'
#   tmux switch-client -t m6
#
...
#   2. Full MV-SFT training on every response that agreed with the majority.
#   3. Serve the adapter and score M6 on val, then test.
set -euo pipefail
cd "$HOME"
mkdir -p logs
exec > >(tee -a "logs/m6_$(date +%Y%m%d_%H%M%S).log") 2>&1
...
for f in data/generations/m6__val.json data/generations/m6__test.json; do
...
stop_vllm
.venv/bin/python main.py train-sft --targets consensus --text-source "$TEXT_SOURCE" --smoke

step "2. Full MV-SFT training"
.venv/bin/python main.py train-sft --targets consensus --text-source "$TEXT_SOURCE"
adapter=$(ls -td runs/*__m6-maca-sft__*/adapter | head -1)
echo "  adapter: $adapter"

step "3. Serve the adapter and score M6 on val, then test"
tmux new-session -d -s vllm-m6 \
  "VLLM_USE_FLASHINFER_SAMPLER=0 .venv/bin/vllm serve $MODEL --port 8000 \
   ...
   --enable-lora --max-lora-rank 16 --lora-modules m6=$adapter \
   2>&1 | tee logs/vllm-m6.log"
wait_for_vllm vllm-m6
for split in val test; do
  .venv/bin/python main.py score --arm m6 --split "$split" --adapter m6
done

.venv/bin/python - <<'PY'
import json
for split in ("val", "test"):
    rows = json.load(open(f"data/generations/m6__{split}.json"))
    ...
    assert rows[0]["model"] == "m6", "scored with the base model, not the adapter"
PY

echo
echo "Done. Next: tmux new -d -s m7 'bash infra/vast/run_m7.sh'"
